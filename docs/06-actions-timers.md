...
Actions are the API game loop. A player writes code to start actions, track them, and react when they complete.
...
| Extract | Convert energy into an ore burst | 1 to 5 minutes |
| Upgrade | Spend ore to improve an upgrade | Instant for MVP |

Passive ore production continues without actions.
