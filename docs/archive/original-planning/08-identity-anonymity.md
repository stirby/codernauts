...
- A player can delete and recreate a local project without losing server state.
- The starter project can call the API without asking the player to manually paste secrets into code.
...
3. The starter project stores the token in an environment variable or local ignored config file.
...
A later version can bind player records to a stable user ID or verified email so local projects can be recreated safely without manual token transfer.

Gameplay requirement:

```text
same player + new local project = same active player record
```
