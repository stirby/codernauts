...
1. Player opens the Codernauts starter project.
2. The starter project provides API URL, API token, starter CLI, examples, and docs.
3. Player runs `codernauts status` or calls `GET /v1/status`.
4. Server returns outpost state, resources, miners, and active actions.
5. Player builds, upgrades, or assigns a persistent miner.
6. Player starts a scan action to discover a new tile.
7. Player polls `GET /v1/actions` or reruns status.
...
Ore rate: +1.0/sec
Scanner: Level 0
Miners: 1 mining at Anchor Rock
Active actions: none
...
- Player upgrades or assigns a miner.
...
- The starter project hides the API behind too much framework code.
